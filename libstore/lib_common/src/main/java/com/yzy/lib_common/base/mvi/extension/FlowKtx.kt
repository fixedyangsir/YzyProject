...
import com.yzy.lib_common.base.mvi.internal.StateTuple2
import com.yzy.lib_common.base.mvi.internal.StateTuple3
...
inline fun <T : UiState> Flow<T>.collectState(
    lifecycleOwner: LifecycleOwner,
    state: Lifecycle.State = Lifecycle.State.STARTED,
    crossinline block: suspend CoroutineScope.(T) -> Unit,
) = lifecycleOwner.lifecycleScope.launch {

    flowWithLifecycle(lifecycleOwner.lifecycle, state).distinctUntilChanged().collect { block(it) }
...
internal fun <T : UiState, A> Flow<T>.collectPartial(
    ...
    lifecycleOwner.lifecycleScope.launch {
        lifecycleOwner.repeatOnLifecycle(state) {
            this@collectPartial.map { prop1.get(it) }.distinctUntilChanged()
...
internal fun <T : UiState, A, B> Flow<T>.collectPartial(
    ...
    lifecycleOwner.lifecycleScope.launch {
        lifecycleOwner.repeatOnLifecycle(state) {
            this@collectPartial.map { StateTuple2(prop1.get(it), prop2.get(it)) }
                .distinctUntilChanged().collect { (partialStateA, partialStateB) ->
...
internal fun <T : UiState, A, B, C> Flow<T>.collectPartial(
    ...
    lifecycleOwner.lifecycleScope.launch {
        lifecycleOwner.repeatOnLifecycle(state) {
            this@collectPartial.map { StateTuple3(prop1.get(it), prop2.get(it), prop3.get(it)) }
                .distinctUntilChanged().collect { (partialStateA, partialStateB, partialStateC) ->
